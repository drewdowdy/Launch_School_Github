...
function total(numbers) {
  let sum; // sum is not initialized to 0; it's undefined

  for (let i = 0; i < numbers.length; i += 1) {
    sum += numbers[i]; // so, doing math on undefined creates NaN
  }

  sum;
}

// To fix this code, we need to make the first line of `total()` to be `let sum = 0;`

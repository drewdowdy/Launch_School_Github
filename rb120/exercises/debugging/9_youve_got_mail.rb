...
Error: on `line 91`, we are invoking the `Object#send` method instead of `Mailing#send`. 

Fix: Mix in the `Mailing` module to `CommunicationsProvider` so that `EmailService`, `TelephoneService`, and `PostalService` all have access to the `Mailing#send` method.

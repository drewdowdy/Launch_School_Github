...
// In the for loop, var declaration is used. This means that the variables are available outside the scope of the for loop. So, in the outer for loop we initialize length to the length of numbers. Then that same length variable is reassigned to the length of cols in the inner for loop. 

// To fix this, we just need to change the var declarations to let declarations so that all the variables are block-scoped.

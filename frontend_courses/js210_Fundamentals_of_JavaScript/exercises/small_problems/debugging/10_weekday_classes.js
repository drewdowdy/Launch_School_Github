...
function toString(date) {
  ...
  function leadingZero(num) { // function to add leading zeros
    return String(num).length === 1 ? `0${num}` : String(num);
  }

  let year = String(date.getFullYear()); // need to get full year
  let month = leadingZero(date.getMonth() + 1); // zero based
  let day = leadingZero(date.getDate()); // changed to getDate()

...
function valenceOfMolecule(...atoms) { // used rest syntax `...atoms`
  let sum = 0;

  // arguments.forEach(atom => {
  atoms.forEach(atom => { // now we can iterate over the `atoms` array
...
// This error occurs because `arguments` is an Array-like object (values can be accessed with indices and has a length property), but it does not have access to any Array.prototype methods like forEach().
// In order to fix this, we could use rest syntax to transform the arugments into a proper Array

...
function random(array, n) {
  ...
  while (n > 0 && elements.length > 0) {
    ...
    elements.splice(randomIndex, 1); // prevents duplicates
    n--;
  }
...
// For dishName and dish, the results are concatinated with +. So, JS converts them into strings so when you call join() on a string, there is a typeerror since that is not possible.
// In order to fix this, we must approach the concatination in a different way. We can use concat() to keep them as arrays.
